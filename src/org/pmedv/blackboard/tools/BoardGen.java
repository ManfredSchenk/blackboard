...
public class BoardGen {
	 ...
	 * 
	 */
	public static BufferedImage generateBoard(int width, int height, BoardType type, boolean solderSide) {
			...
			if (type != null && type.equals(BoardType.HOLES) && solderSide) {
				...
				for (int x = RASTER; x < width; x += RASTER) {
					for (int y = RASTER; y < height; y += RASTER) {
						g2.fillOval(x - 2 - PADSIZE / 2, y - 2 - PADSIZE / 2, PADSIZE+4, PADSIZE+4);
					}
				}
